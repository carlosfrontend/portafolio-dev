    ...
    () => {
      document.fonts.ready.then(() => {
        if (
          ...
          console.warn(
            "Algunas referencias de elementos no están disponibles para la animación principal.",
          );
          return;
        }

        gsap.set(
          [titleRef.current, subtitleRef.current, linkButtonRef.current],
          { clearProps: "all" },
        );

        splitTextTitleInstance.current = SplitText.create(titleRef.current, {
          type: "words",
        });
        splitTextSubtitleInstance.current = SplitText.create(
          subtitleRef.current,
          { type: "words" },
        );

        gsap.set(splitTextTitleInstance.current.words, {
          y: 10,
          autoAlpha: 0,
          filter: "blur(3px)",
        });
        gsap.set(splitTextSubtitleInstance.current.words, {
          y: 8,
          autoAlpha: 0,
          filter: "blur(2px)",
        });
        ...
        tl.to(
          splitTextTitleInstance.current.words,
          {
            y: 0,
            autoAlpha: 1,
            filter: "blur(0px)",
          ...
          .to(
            splitTextSubtitleInstance.current.words,
            {
              y: 0,
              autoAlpha: 1,
              filter: "blur(0px)",
  ...
  return (
    <section className="relative min-h-[100dvh] flex flex-col justify-start md:top-10 items-center">
      <div className=" w-full md:w-[480px] shadow-base-300 flex flex-col justify-between md:shadow-md  shadow-none md:bg-base-100 rounded-md">
        <a
          href={"https://www.linkedin.com/in/carloshelpdesk/"}
        ...
        <div className="card-body w-full  items-center justify-center text-center text-primary-content p-4">
          <h2
            ref={titleRef}
            className=" text-pretty text-base-content text-4xl font-extrabold"
          >
            Hola, soy Carlos Pulido
          </h2>
          <h3
            ref={subtitleRef}
            className="text-2xl text-base-content font-medium py-4"
          >
          ...
          <Link
            ref={linkButtonRef}
            href="#about"
            className="btn btn-primary w-full md:w-auto py-6 text-lg md:text-sm md:py-0 "
          >
            Conóceme
          </Link>
        </div>
        ...
        <div className="flex w-full p-4 md:w-full self-center gap-4 justify-center items-stretch  flex-wrap flex-col md:flex-row">
          <a
            href={"/CV_Carlos_Pulido_Soporte_IT_Helpdesk_2026-02-13_2209.pdf"}
            target="_blank"
            rel="noopener noreferrer"
          >
          ...
          <a
            href={"mailto:carlos.pulido.tech@outlook.com"}
            rel="noopener noreferrer"
          >
