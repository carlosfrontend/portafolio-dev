...
export default async function Page() {
  const { data }: { data: POSTS_QUERYResult } = await sanityFetch({
    query: POSTS_QUERY,
  });
  const posts = data;
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-10 md:gap-8 p-6 md:p-8 bg-base-100 w-full justify-items-center">
        {posts.map((post) => (
